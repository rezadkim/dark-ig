...
termux() {
    ...
    echo
    echo -e "${W}[ ${B}INFO${W} ] ${G}darkig${W}: upgrading ${Y}pip${W} ..."
    echo
    python3 -m pip install --upgrade pip
    echo
    echo -e "${W}[ ${B}INFO${W} ] ${G}darkig${W}: install ${Y}requirements.txt${W} with pip ..."
    echo
    cd $folder || exit 1
    python3 -m pip install --trusted-host pypi.org --trusted-host files.pythonhosted.org -r requirements.txt
    echo
    echo -e "${W}[ ${B}INFO${W} ] ${G}darkig${W}: installing ${Y}dark-ig${W} ..."
    echo
